...
those bytes *were*, and the two possible answers call for opposite fixes.
...
Coalescing is per *transaction*, not per append. `append_replay` runs once per
...
*globally oldest* row mirrors the whole-row path, but `ordinal` is an insertion
...
neighbour keeps everything. Shedding the *largest* Run toward an equal share is
...
The slice test also re-sends the same range with *different* bytes and requires
...
*base* arm. R11 recorded zero across all 28 of its arms.
...
`replay_bytes` matches both. The first version of this check ran the *quiet*
