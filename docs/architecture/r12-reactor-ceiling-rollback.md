...
16 deliver the *same* 53 MB/s, which is a hard ceiling rather than a workload
levelling off.
...
noise by the spread test *and* split their pairs; these lose every pair.
...
So the change did exactly what it was designed to do, and that is *why* the
...
structure of this system: *the reactor is the fleet's rate limiter, and
...
- **Bound admission, not render cost.** The reactor already skips the render
  when `queue_has_room()` is false. It does not slow down *reading*, so the
