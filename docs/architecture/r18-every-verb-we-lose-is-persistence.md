...
tmux runs in the same batch, because the question is how much of the gap *to
tmux* persistence explains — not how much faster memory-only is than itself.
...
The share also *grows* with output volume (start 85.0% -> 86.9%, stop+remove
...
What the floor *does* establish is that the reactor, PTY and spawn paths are
