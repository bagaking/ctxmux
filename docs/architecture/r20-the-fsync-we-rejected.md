...
> p90.** R19 made the fold _rarer_, not _cheaper_. At c8 the WAL peaks ~8.5 MB,
...
**Wrong on the verb, right on the statistic.** The gate _was_ p90 — that is what
...
Pooled p90 (180 observations per arm, stratified bootstrap resampling _within_
round, 95% CI):
...
NORMAL makes the typical `stop` _faster_ (c2 7.97→5.63, c8 11.26→6.92) and
...
First, what is _not_ true, because I wrote it down before checking: it is not
...
What _is_ specific to `stop` is the **derivative**: when persistence gets
...
- `start` and `remove` block on **their own** commit. NORMAL removes an fsync
  from that commit, so both improve — medians _and_ tails, at every shape.
...
optimization that makes the persistence actor cheaper _per unit of work_ buys
...
asymmetry _worse_. It speeds up exactly the commits that deepen the queue while
...
the _same binary_ differed by 20 ms on `start_p90`. I had gated the round on a
...
bootstrapping _within_ round recovered a usable gate from data already on disk.
...
identifies the mechanism — is the _lightest_ workload, the one most tempting to
