...
`require_waitable_anchor` keeps true — so "no child has exited" *proves* no
...
specifically *two separate full commits* — stop's finalize and remove's own
...
*scaling* defect, and the distinction matters: holding records fixed at 256 and
...
* the four small-fleet cells are all inside their floors. The pre-registration
  ...
  exactly that shape *and it is the shape the tmux comparison uses*. One added
...
consume it, but it returns *the same pid* until that child is reaped. Eight
...
* **stale `false`** would be the dangerous one: an exit arriving after the read.
  ...
  gate. The byte is *buffered*, not edge-triggered, and the owner drains the
...
untraced scaling pass disagreed. Tracing prices syscall *counts*, never time.
...
the control is what makes the proof readable: a no-op recompile of *identical*
