...
emptiness as an *event*, and `cgroup.kill` would also catch `setsid` escapees.
...
*started* inside a delegated scope (`systemd-run --user --scope -p
...
The sweep is proportional to *our* tree, not the host's. That is the whole
...
`strace -c` a real `stop`. `openat("/proc")` for the *host walk* must go 1 → 0.
...
exactly on a process that is *in the session but not in our tree*.
...
The remaining row is the one no fixture can enumerate: *a process that joined
the session without descending from the leader*. It cannot exist —
...
*does* keep native control, and those children are still ours across `execve` —
