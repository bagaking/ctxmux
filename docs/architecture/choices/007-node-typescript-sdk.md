...
The SDK now validates full nested generation-2 frames, rejects unsafe cursors, bounds queued input to 256 frames or 1 MiB before pausing the socket, and waits for `Detached` on clean detach. Short request methods wait for `Response` or `Error`. `Attachment.input()`, `resize()`, and `stop()` currently resolve after their frame's socket-write callback; remote `Accepted` or `Error` remains a separate attachment event because generation 2 has no command correlation ID.
