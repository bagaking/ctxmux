...
| Persistent historical recovery                           | Real daemon restart restores exited replay/lineage and Level A fork; live rows become interrupted; typed `DiskFull` actor faults retry append/finalize in order; stale unrelated PID, lock, schema, corrupt generation, modes, symlinks, and retention fail closed | Strong for the declared historical class and transient storage exhaustion | Kill-at-every-SQLite-transition injection, power-loss filesystems, live PTY handoff, and migration remain unsupported |
...
compile failures are reported as platform-impossible because generation 9
...
- **Earned streaming optimization:** freeze an incremental raw-byte consumer
  workload before changing generation-9 integer-array encoding. Adopt a new
