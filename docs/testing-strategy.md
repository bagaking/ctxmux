...
| Runtime identity and capability dispatch fences          | Public Rust and TypeScript client tests replace Hello identity/capabilities and prove Request, Attach, and attachment-control paths return typed local failures after Hello while sending zero business frames                                                                                                                                                                             | Strong for first-party generation-14 clients                              | Endpoint selection, peer credentials, and host or Provider identity remain outside this client-local fence                          |
...
| Chaos, security, and resources                           | Named process kills, malformed/oversized/long-lived frames, argv/env/secret negatives, concurrent start, churn, and idle/active 1/32/128 census                                                                                                                                                                                                                                            | Strong for the bounded native generation-14 matrix                        | Hostile parent/FD exhaustion, attachment admission, total RSS quota, and production-scale pressure remain open                      |
...
| Persistent historical recovery                           | Real cold daemon restart restores exited replay/lineage and Level A fork; external generation writes stay outside the main WAL; a sparse generation past the 384 MiB SQLite ceiling compacts; rollback tails are truncated on reopen; oversized, missing, overlapping, and truncated extents fail closed; typed `DiskFull`, write-side I/O pressure, and `StorageFull` retry append/finalize in order                                                                 | Strong for deterministic generation recovery and transient storage exhaustion | Kill-at-every-SQLite-transition injection, power-loss filesystems, cross-process live PTY handoff, and migration remain unsupported |
| Planned exec-in-place upgrade continuity                 | Real persistent daemon SIGHUP fixtures preserve Runtime/daemon identity, child PID, and listener inode; prove ordered replay, Run-local Input cursor/ledger response-loss retry, recoverable Stop receipt replay, crossing Input ACK before reconnect, draining rejection as `not_applied`, reversible pre-extract abort, signal-exit identity, and repeated settled fd/thread/path census | Strong for same-process generation-14 exec upgrade                        | Cold/crash continuity and compatibility across a future protocol/schema change remain separate claims                               |
...
compile failures are reported as platform-impossible because generation 14
...
- **Earned streaming optimization:** the generation-13 integer-array workload
  was frozen before the wire change, and generation 14 now carries strict
  padded base64 with one-shot SDK decoding. The frozen reliability budgets stay
  unchanged; exact bytes, byte cursors, replay, `Gap`, frame limits,
  backpressure, security, and resource bounds remain the acceptance oracles.
