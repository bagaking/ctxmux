...
| Persistent historical recovery                           | Real cold daemon restart restores exited replay/lineage and Level A fork; live rows become interrupted; typed `DiskFull` actor faults retry append/finalize in order; stale unrelated PID, lock, schema, corrupt generation, modes, symlinks, and retention fail closed                                                                                                                    | Strong for the declared historical class and transient storage exhaustion | Kill-at-every-SQLite-transition injection, power-loss filesystems, cross-process live PTY handoff, and migration remain unsupported |
