...
fn openpty(size: PtySize) -> anyhow::Result<(UnixMasterPty, UnixSlavePty)> {
    ...
    // walk this fork already fixes. The daemon never asks for the name on any
    // production path, so it paid the largest single item on its Run-creation
    // path for a string it discards. Resolved lazily in `tty_name()` instead,
    // from the master with `ptsname`, which is a pointer read out of the
    // kernel's own pty table: 0.01ms, flat, and byte-identical to the eager
    // answer in 300/300 pairs. `ptsname` also keeps working after the slave fd
    // is closed, which is what every spawn does, so the deferral does not
    // narrow the window in which the name is available.
    //
    // Caching the eager answer in a field to save an allocation per `tty_name()`
    // call is a bad trade in the other direction: it buys an allocation on a
    // call with no production callers by paying a /dev walk on every spawn.
